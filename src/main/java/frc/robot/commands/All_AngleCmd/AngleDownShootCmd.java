...
public class AngleDownShootCmd extends Command { // Déclaration de la classe AngleDownManualCmd qui étend la classe Command
    ...
    public AngleDownShootCmd(AngleSubsystem angle) { // Constructeur de la classe AngleDownManualCmd
    ...
    public boolean isFinished() { // Méthode qui indique si la commande est terminée
        if(m_robotContainer.Potentio.getValue() <= 4065){
            return true;
